...
function cellClicked(elCell, event, i, j) {
    ...
    // If LeftClick Proceed as usual:
        ...
        } // If this is the first click: start game
        ...
        // If cell is marked OR shown do not proceed:
        if (gBoard[i][j].isMarked || gBoard[i][j].isShown) return;

        // If cell is a mine:
        if (gBoard[i][j].isMine) {
            // If this is the first click REGENERATE BOARD!
                    ...
                    for (let i = 0; i < gBoard.length; i++) {
                        for (let j = 0; j < gBoard[i].length; j++) {
                            ...
                            if (!gBoard[i][j].isShown) {
                                ...
                                var currCellContent = (gBoard[i][j].isMine) ? MINE : gBoard[i][j].minesAroundCount;
                                elCurrCell.innerText = currCellContent;
                            }
                        }
                    }
                }
        ...
        // If cell is not a mine (SWEET!)
            ...
            if (gGame.showCount === gLevel.size * gLevel.size - gLevel.mines) {
                ...
                clearInterval(gGame.intervalId);
                // Keep record:
                keepScore(gGame.secsPassed);


                // Add flags where necessary:
                for (let i = 0; i < gBoard.length; i++) {
                    for (let j = 0; j < gBoard[i].length; j++) {
                        // If this is a mine + was not flagged:
